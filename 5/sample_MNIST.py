...
if __name__ == '__main__':
    ...
    while three_cases<3:
        ...
        for i in range(5):
            ...
            if i==0:
                display(train_acc,test_acc,"MNIST Classification with "+param["optimizer"]+" and "+str(param["Learning_Rate"])+" learning rate","Epochs","Accuracy")
        ...
        writedattofile("test.txt",param,list(test_np[:,-1]),np.amax(test_np, axis=0)[-1],np.mean(test_np, axis=0)[-1],np.std(test_np, axis=0)[-1],"MNIST")
        three_cases+=1
