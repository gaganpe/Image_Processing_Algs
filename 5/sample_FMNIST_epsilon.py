...
def load_data(device,epsilon):
    global train_set_orig_targets
    ...
    train_set_orig_targets = train_set.targets
    train_set_new_targets = train_set.targets.clone()
    for i in range(len(train_set.targets)):
        random_number = np.random.random()
        if random_number < epsilon :
            orig = int(train_set.targets[i])
            other_nums = [a for a in range(10) if a != orig]
            random_element = np.random.choice(other_nums)
            train_set_new_targets[i] = torch.tensor(random_element)
    train_set.targets = train_set_new_targets
...
if __name__ == '__main__':
    class_n = ['T-shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat', 'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot']
    ...
    final_acc_l = []
    std_l = []
    for epsilon in [0,0.2,0.4,0.8]:
        epsilon_l.append(epsilon)
        train_acc_list = []
        test_acc_list = []
        param = {}
        param["epsilon"] = epsilon
        train_loader,test_loader=load_data(device,epsilon)
        current_tar = train_loader.dataset.targets.numpy()
        test_cm = confusion_matrix(train_set_orig_targets.numpy(), current_tar)
        cm_norm = test_cm.astype('float') / test_cm.sum(axis=1)[:, np.newaxis]
        plot_cm(cm_norm,"Noise in Training Data with Epsilon = "+str(epsilon)) 
        ...
        for i in range(5):
            ...
            if(three_cases == 2):
                ...
                param["Learning_Rate"] = 0.0007
                #momentum=0.9
                weight_decay  = 0.00002
                param["weight_decay"] = 0.00002
                criterion=nn.CrossEntropyLoss()
            ...
            if i==0:
                display(train_acc,test_acc,"FashionMNIST Classification with "+param["optimizer"]+" and "+str(param["Learning_Rate"])+" learning rate","Epochs","Accuracy")
                ...
                cm_norm = test_cm.astype('float') / test_cm.sum(axis=1)[:, np.newaxis]
                plot_cm(cm_norm,"Confusion Matrix for Test Data with Epsilon = "+str(epsilon))        
                ...
                for idx in top_3_indices:
                    ...
                    x,y = test_loader.dataset[wrong_idx]
                    x_wrong = x.cpu().detach().numpy()
                    x_wrong = np.squeeze(x_wrong, axis=0)
                
                    plt.imshow(x_wrong, cmap='gray')
                    title1 = "{}to{}.png".format(class_n[y],class_n[test_predictions[wrong_idx]])
                    plt.title("{} Test Label mislabed as {}".format(y,test_predictions[wrong_idx]))
                    #plt.show()
                    plt.savefig(title1, dpi=300)
                total_auc = 0
                """for class_id in set(test_labels):
                    class_of_interest = str(class_id)
                    ...
                    plt.title("One-vs-Rest ROC curves:\n{} vs Rest".format(class_of_interest))
                    plt.legend()
                    plt.show()
        ...
        mean_acc_l.append(np.mean(test_np, axis=0)[-1])
        final_acc_l.append(np.amax(test_np, axis=0)[-1])
        std_l.append(np.std(test_np, axis=0)[-1])
        writedattofile("test_FMNIST.txt",param,list(test_np[:,-1]),np.amax(test_np, axis=0)[-1],np.mean(test_np, axis=0)[-1],np.std(test_np, axis=0)[-1],"FMNIST")
        three_cases+=1
    f = open("test_FMNIST.txt", "a")
    f.write('---------------------------------------------\n')
    
    f.write("Epsilon val = {}\n".format(str(epsilon_l)))
    f.write("Mean Acc= {}\n".format(str(mean_acc_l)))
    f.write("STD val = {}\n".format(str(std_l)))
    f.write("Final Acc val = {}\n".format(str(final_acc_l)))
